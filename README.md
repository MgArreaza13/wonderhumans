...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv wonderhumans` or `python -m venv wonderhumans-venv`
...
- [ ] Make sure the virtualenv is activated `workon wonderhumans` or `source wonderhumans-venv/bin/activate`
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `wonderhumans/backend/wonderhumans/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `wonderhumans/backend/wonderhumans/settings/production.py`
...
- Create a copy of ``wonderhumans/settings/local.py.example``:  
  `cp wonderhumans/settings/local.py.example wonderhumans/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/wonderhumans/settings/base.py``.
- Adjust ``backend/wonderhumans/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
